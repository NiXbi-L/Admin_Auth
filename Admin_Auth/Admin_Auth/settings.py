...
SECRET_KEY = os.environ.get('SECRET_KEY', 'django-insecure-g1nu3!66#&p=sslk32ui9_s%ad4$c0a=#sn%752yq5+n^b9d(%')
...
CORS_ALLOW_METHODS = os.environ.get('CORS_ALLOW_METHODS', 'GET,POST,PUT,PATCH,DELETE,OPTIONS').split(',')

# CORS_ALLOW_ALL_ORIGINS будет использоваться, если указано значение "True"
...
MIDDLEWARE = [
    ...
    'corsheaders.middleware.CorsMiddleware',  # CORS middleware (должен быть перед Common middleware)
...
# SQLite fallback if PostgreSQL is not configured
if any(not os.environ.get(key) for key in ['DB_NAME', 'DB_USER', 'DB_PASSWORD']):
    DATABASES = {
        'default': {
...
SIMPLE_JWT = {
    ...
    'UPDATE_LAST_LOGIN': os.environ.get('JWT_UPDATE_LAST_LOGIN', 'False') == 'True',
